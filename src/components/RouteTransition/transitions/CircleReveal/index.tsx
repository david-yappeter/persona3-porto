...
/*
 * Renders the outgoing page behind, fully visible and unclipped, and the
 * live/incoming page (from props.outlet) on top, clipped down to the union
 * of two circles that grow from 0 to full-cover — so it looks like the old
 * page gets progressively painted over by the new one, revealed through the
 * growing circles.
 *
 ...
 */
export const CircleRevealTransition = ({ outlet, exiting, onExitComplete }: RouteTransitionEffectProps) => {
  useEffect(() => {
    const ghostVideo = document.querySelector<HTMLVideoElement>('.route-exit-old video')
    if (ghostVideo) {
      ghostVideo.currentTime = exiting.videoTime
      void ghostVideo.play()
    }
    const timer = setTimeout(onExitComplete, 950)
  ...
  return (
    <>
      ...
      <div className="route-live route-enter" style={{ clipPath: `url(#${clipId})` } as CSSProperties}>
        {outlet}
      </div>
